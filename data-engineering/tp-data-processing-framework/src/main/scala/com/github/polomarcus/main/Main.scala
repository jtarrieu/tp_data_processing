...
object Main {
  def main(args: Array[String]) {
    ...
    // Enrich the dataset by applying the ClimateService.isClimateRelated function to the title and the description of a news
    ...
    val query = spark.sql("SELECT count(*) as nb_rows_climate FROM news WHERE containsWordGlobalWarming=true")
    query.show()
